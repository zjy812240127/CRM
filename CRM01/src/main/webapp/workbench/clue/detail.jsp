...
<script type="text/javascript">
	...
	$(function(){
		...
		$("#aname").keydown(function (event){
			...
			// 如果按下的是回车，进行搜索
			if(event.keyCode == 13){
				alert("执行市场活动查询操作，并展现查询结果")
				$.ajax({
					url:"searchActivity.do",
					data:{
					...
					$("#linkActivityBody").html(html);

					}
				})
				// 模态窗口默认敲回车会清空当前窗口数据，所以以上做完后要禁用这个功能
				return false;
			}
		})

		// 为关联市场活动的模态窗口关联按钮绑定事件
		$("#bundBtn").click(function (){
			...
			if ($xz.length == 0){
				...
				// 一定选择了市场活动，但是可能是一个或者多个
				// param = clueId + activityId  + activityId + activityId...
				// 地址栏传递参数时，不能有空格
				var param = "cid=${clue.id}&"

				for (var i=0; i<$xz.length; i++){
					// jquery对象[下标]转换为dom对象，$(dom对象)转化为jquery对象
					param += "aid=" + $($xz[i]).val()
					// 最后一个aid之前都要加&分隔符
					if (i < $xz.length -1 ){
						param += "&"
				...
				$.ajax({
					...
					success: function (data) {
						...
						if (data.success) {
							...
							// 清空输入关键字文本框， 清空全选框和单选框， 清空模态窗口linkActivityBody的内容
	...
	function showActivityList(){
		/**
		 * 加载市场活动
		 *      读取域中的clue.id传输到后端，查询市场活动线索关联表，获得对应的activityId
		 *      根据市场活动id查找市场活动表中对应的数据行，
		 *      将数据行信息传导前端，铺到市场活动列表
		 */
		$.ajax({
			...
			success:function (data){
			//	alert("接收到后端查询到的市场活动")
			//	alert("后端返回的市场活动的个数"+data.length)
				/**
				 * data:
				 *      [{市场活动1}，{市场活动2}，{市场活动3}。。。]
				 *      owner要两表联查
	...
	function unbund(id){
		/**
		 * 此处得到的id是市场活动与线索关联记录行的id
		 * 		根据这个id可以删除数据库中关联表的相应数据行
		 */
		// alert(id)
		$.ajax({
			url:"unbund.do",
